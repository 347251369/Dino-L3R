"""Full-view GSP training for the DINOv3 abdominal CT model.

The final recipe uses balanced sampling, recall-aware overlap supervision, a
slice-level organ-presence objective, and a zero-initialized depth adapter.
"""
...
class OrganBalancedEpochSampler(Sampler[int]):
    """Draw target organs first, then draw slices containing that organ.

    The same frequency-derived rule is used for every dataset. A fixed uniform
    fraction retains background and ordinary anatomy so the model does not
    overfit to only rare-organ slices.
    """
...
class CoverageBalancedEpochSampler(Sampler[int]):
    """Cover every slice once, then add uniformly organ-balanced samples.

    The supplement size follows from the requested epoch length rather than a
    manually tuned sampling percentage. Organ targets are cycled uniformly and
    the final draw order is shuffled deterministically for each epoch.
    """
...
class PresentClassTverskyLoss(nn.Module):
    """Recall-aware overlap loss evaluated only for present foreground classes."""
...
class BalancedPresenceBCELoss(nn.Module):
    """Balance present and absent organ decisions within each slice batch."""
