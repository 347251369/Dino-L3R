"""Run deterministic full-volume inference with a trained GSP-DINO checkpoint."""
...
def load_case_data(item: tuple[int, str]) -> tuple[int, str, np.ndarray, np.ndarray, nib.Nifti1Header, float]:
    """Load one complete case so disk I/O can overlap GPU inference."""
...
def iter_prefetched_cases(
    ...
    """Yield cases with a bounded number of background loads in flight."""
...
def save_case_artifacts(
    ...
    """Persist one case; intended to run in the bounded I/O pool."""
    if save_hard:
        nib.save(
